...
import { House, Users, Calendar, BarChart3, FolderKanban, FolderOpenDot, FileText, Cog, Contact, User, ChartColumn } from 'lucide-react';
...
const menuItems = [
...

  {
    ...
    subItems: [
      { name: "Help & Support", path: "/admininistration/help_support" , subMenu: [
        { name: "FAQs", path: "/admininistration/help_support/faqs" },

      ]},
      { name: "User Management", path: "/admininistration/user_management", subMenu: [
        { name: "Users", path: "/admininistration/user_management/users" },
        { name: "Roles &Permissions", path: "/admininistration/user_management/roles_permissions" },
        ...
        <>
            <div 
                className={`fixed inset-0 bg-black/30 backdrop-blur-sm transition-opacity duration-300 ${
                    !collapsed && isMobile ? 'opacity-100 z-20' : 'opacity-0 pointer-events-none'
                }`}
                                ...
                                >
                                    <Link 
                                        ...
                                        className={`group flex items-center gap-x-3 px-3 py-2 rounded-lg transition-all duration-150
                                            ${openMenus[item.name] ? 'bg-indigo-50 text-indigo-600' : 'hover:bg-gray-50'}
                                        `}
                                        onClick={(e) => {
                                            if (item.subItems) {
                                    ...
                                    >
                                        <span className={`flex-shrink-0 transition-colors duration-150
                                            ${openMenus[item.name] ? 'text-indigo-600' : 'text-black group-hover:text-gray-700'}
                                        `}>
                                            {item.icon}
                                        </span>
                                        ...
                                        {!collapsed && (
                                            <>
                                                <span className={`font-medium transition-colors duration-150
                                                    ${openMenus[item.name] ? 'text-indigo-600' : 'text-black group-hover:text-gray-900'}
                                                `}>
                                                    {item.name}
                                                </span>
                                                    ...
                                                    <div className="relative">
                                                        <Link 
                                                            ...
                                                            className={`group flex items-center justify-between py-2 pl-3 pr-2 rounded-md text-sm transition-colors
                                                                ${openSubMenus[subItem.name] ? 'text-indigo-600 bg-indigo-50' : 'text-black hover:text-gray-900 hover:bg-gray-50'}
                                                            `}
                                                            onClick={(e) => {
                                                                if (subItem.subMenu) {
                                                        ...
                                                        >
                                                            ...
                                                            {subItem.subMenu && (
                                                                <span className={`transition-transform duration-200 ${
                                                                    openSubMenus[subItem.name] ? 'rotate-180' : ''
                                                                }`}>
                                                                    <FaAngleDown size={12} />
                                                                </span>
                                                            )}
