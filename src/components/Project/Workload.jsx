...
const Workload = () => {
  ...
  const [chartData, setChartData] = useState([
    {
      period: "01 Apr - 04 Apr",
      availableCapacity: 65,
      assignedAllocations: 20,
      unassignedAllocations: 0,
      totalCapacity: 75,
    },
    {
      period: "07 Apr - 11 Apr",
      availableCapacity: 75,
      assignedAllocations: 0,
      unassignedAllocations: 0,
      totalCapacity: 75,
    },
    {
      period: "14 Apr - 18 Apr",
      availableCapacity: 75,
      assignedAllocations: 0,
      unassignedAllocations: 0,
      totalCapacity: 75,
  ...
  const [resources, setResources] = useState([
    ...
    {
      id: "avsm",
      role: "AV Subject Matter Expert",
      avatar: "AV",
      color: "bg-red-500",
    },
    {
      id: "avse",
      role: "AV Support Engineer",
      avatar: "AV",
      color: "bg-red-500",
    },
  ...
  const updateAllocation = (resourceId, periodId, value) => {
    const newValue = prompt(
      `Update allocation for ${resourceId} during ${periodId}:`,
      value
    );
    if (newValue) {
      setAllocations((prev) => ({
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className={`${darkMode ? "card-dark" : null}  p-4 shadow-sm mb-4`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium">
            Allocations (demand) vs Capacity (supply)
          </h2>
        ...
        <div className="text-end">
          <input
            ...
            }  px-3 py-1 border rounded-md pr-8 gap-2 w-25 text-end`}
          />
        </div>
        ...
        <div className="w-full h-64">
          ...
          >
            ...
            />
            <Bar
              dataKey="assignedAllocations"
              fill="#38B2AC"
              name="Assigned allocations"
              stackId="a"
            />
            <Bar
              dataKey="unassignedAllocations"
              fill="#9AE6B4"
              name="Unassigned allocations"
              stackId="a"
            />
          </BarChart>
        </div>
      ...
      >
        ...
        >
          ...
          <thead>
            <tr className="">
              <th className="py-2 px-4 border-b border-r font-medium  text-left w-48">
                Role
              </th>
              {Object.entries(groupedPeriods).map(
                ([month, periods], monthIndex) => (
                  <th
                    key={month}
                    colSpan={periods.length}
                    className="py-2 px-4 border-b font-medium  text-center"
                  >
                    {month}
                  </th>
                )
            ...
            <tr>
              ...
              {timePeriods.map((period) => (
                <th
                  key={period.id}
                  className="py-2 px-2 border-b font-medium  text-center whitespace-nowrap w-16"
                >
                  {period.label}
                </th>
              ))}
            ...
            {resources.map((resource) => (
              <tr key={resource.id} className="">
                <td className="py-2 px-4 border-b border-r">
                  <div className="flex items-center">
                    <div
                      className={`w-6 h-6 rounded-full ${resource.color}  flex items-center justify-center mr-2 text-xs font-medium`}
                    >
                      {resource.avatar}
                    </div>
                ...
                {timePeriods.map((period) => (
                  <td
                    ...
                    }  py-2 px-2 border-b text-center cursor-pointer ${getAllocationCellColor(
