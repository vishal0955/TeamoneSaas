...
const CalendarApp = () => {
  ...
  const [events, setEvents] = useState([
    ...
    },
    {
      ...
      start: new Date(
        new Date(dayAfterTomorrow).setHours(10, 0, 0)
      ).toISOString(),
      end: new Date(
        new Date(dayAfterTomorrow).setHours(11, 0, 0)
      ).toISOString(),
      color: "#FF9800",
    },
  ]);
  ...
  const handleDateClick = (info) => {
    ...
    setNewEvent({
      ...
      endTime: new Date(clickedDate.getTime() + 45 * 60000)
        .toTimeString()
        .substring(0, 5),
    });
  ...
  const renderMiniCalendar = () => {
    ...
    return (
      <div
        ...
        } mb-4 p-4 border-t border-gray-700`}
      >
  ...
  return (
    <div
      className={` ${
        darkMode ? "dark-mode" : null
      } flex flex-col md:flex-row h-screen bg-gray-100 `}
    >
      {/* Mobile Menu Toggle */}
      <button
      ...
      <div
        className={` ${
          darkMode ? "dark-mode" : null
        }  fixed inset-y-0 left-0 w-64 z-10 transform transition-transform duration-300 ease-in-out mt-5
          ${
      ...
      <div className={` flex-1 p-4 overflow-hidden mt-16 md:mt-0`}>
        <div
          className={`flex-1 ${
            darkMode ? "dark-mode" : null
          } rounded-lg shadow p-4 h-full flex flex-col`}
        >
          {/* Calendar Header */}
              ...
              <h2 className="text-xl font-bold">
                {currentView === "timeGridDay"
                  ? selectedDate.toLocaleDateString("en-US", {
                    year: "numeric",
                    day: "numeric", // Add weekday (e.g., Monday)
                      month: "long", // Month in long format (e.g., January)
                       // Day of the month (e.g., 1)
                      // Year in numeric format (e.g., 2025)
                    })
                  : selectedDate.toLocaleDateString("en-US", {
